...
class Help:
    ...
    def getSpecificHelp(self, select):
        if select == 'game':
            ...
            em.add_field(name='st!game search ( query )', value='`query` 자리에 입력한 문장으로 스팀을 검색해요.')
            ...
            em.add_field(name='st!user profile ( username )', value='`username` 자리에 입력한 유저의 프로필을 가져와요.')
            em.add_field(name='st!user recent ( username )', value='`username` 자리에 입력한 유저가 최근 2주간 플레이한 게임을 가져와요.')
            em.add_field(name='st!user library ( username ) ', value='`username` 자리에 입력한 유저의 라이브러리를 10개 가져와요. 명령어 뒤에 숫자를 붙이면 입력한 만큼 가져와요. (최대 25개)')
            em.add_field(name='st!user wishlist ( username )', value='`username` 자리에 입력한 유저의 찜 목록를 10개 가져와요. 명령어 뒤에 숫자를 붙이면 입력한 만큼 가져와요. (최대 50개)')
